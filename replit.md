...
- **CRITICAL DEPLOYMENT FIX (Nov 15, 2025)**: Fixed Railway deployment issue causing multiple bot instances to run simultaneously. Solution: (1) Added `numReplicas: 1` to `railway.json` to enforce single instance, (2) Created `reset_state.py` script for complete state cleanup, (3) Added healthcheck configuration to prevent premature deployments, (4) Reduced `restartPolicyMaxRetries` from 10 to 3, (5) Added Gunicorn as production WSGI server for Flask dashboard. This eliminates Telegram API conflicts (409 errors) and database connection issues. See `RAILWAY_FIX.md` for complete remediation guide.
